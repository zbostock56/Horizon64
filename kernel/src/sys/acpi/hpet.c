 ...
 *        general configuration register
 */
static inline void hpet_enable() {
...
/**
 * @brief Reads the COUNTER_CLK_PERIOD field in the HPET general capabilities
 * register
 *
 * @return uint32_t Value stored in that field
 */
static inline uint32_t hpet_read_counter_clk_period() {
    return ((hpet->general_capabilities << HPET_COUNTER_CLK_PERIOD) & 0xFFFF);
}

/**
 * @brief Get nanoseconds since HPET intialization
 *
 * @return uint64_t Number of nanoseconds since HPET initialization
 */
uint64_t hpet_get_nanos() {
    ...
    return hpet->main_counter_value * hpet_period;
}

/**
 * @brief Gets number of milliseconds since HPET initialization
 *
 * @return uint64_t Number of milliseconds since HPET intialization
 */
...
/**
 * @brief HPET specific sleep function
 *
 * @param millis Number of milliseconds to sleep
 */
void hpet_sleep(uint64_t millis) {
    uint64_t end = hpet_get_millis() + millis;

    while (hpet_get_millis() < end) {
        asm volatile("pause;");
    }
}
...
STATUS hpet_init() {
    ...
    klogd("INIT HPET: %d comparators total\n", num_hpet_comparators);
    ...
    uint32_t counter_clk_period = hpet->general_capabilities >> 32;
    uint64_t freq = 1000000000000000 / counter_clk_period;

    klogd("INIT HPET: HPET COUNTER_CLK_PERIOD field value: %d\n",
           hpet_read_counter_clk_period());

    klogd("INIT HPET: HPET frequency detected as %d HZ\n", freq);
