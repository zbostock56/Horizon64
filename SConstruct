...
from build_helpers.phony_targets import PhonyTargets
from build_helpers.utility import ParseSize, RemoveSuffix
...
if HOST_ENVIRONMENT['config'] == 'debug':
    ...
    HOST_ENVIRONMENT.Append(CCFLAGS = ['-O2'])
...
toolchainDir = Path('toolchain', RemoveSuffix(platform_prefix, '-')).resolve()
...
PhonyTargets(HOST_ENVIRONMENT, 
             ...
             toolchain=['./scripts/setup_toolchain.sh'])
